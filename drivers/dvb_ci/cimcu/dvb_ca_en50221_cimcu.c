...
do {\
	if (dvb_ca_en50221_debug)\
		pr_err("dvb_ca_en50221_cimcu: " fmt, ## args);\
} while (0)
...
static int dvb_ca_en50221_check_camstatus(struct dvb_ca_private *ca, int slot)
{
	...
	if (!cam_changed) {
		cam_present_old = (ca->slot_info[slot].slot_state != DVB_CA_SLOTSTATE_NONE);
...
static int dvb_ca_en50221_link_init(struct dvb_ca_private *ca, int slot)
{
	...
	ret = ca->pub->write_cam_control(ca->pub, slot, CTRLIF_COMMAND, IRQEN | CMDREG_SR);
	if (ret != 0)
		return ret;
	ret = dvb_ca_en50221_wait_if_status(ca, slot, STATUSREG_DA, HZ / 2);
	if (ret != 0)
		return ret;
	...
	ret = ca->pub->write_cam_control(ca->pub, slot, CTRLIF_COMMAND, IRQEN | CMDREG_SW);
	if (ret != 0)
		return ret;
				     ...
				     int *address, int *tupleType, int *tupleLength, u8 *tuple)
{
	...
	if (_tupleType < 0) {
		dprintk("read_attribute_mem error\r\n");
		return _tupleType;
	}
...
static void parse_ciplus_compatibility(char *tuple, int len, int *ci_plus_enabled)
{
	char flag = '+';
	int number;
	int i;
	...
	if (flag == '-' && number == 1) {
		/* CI+ "v1" not supported */
		dprintk("--tuple[%s]=set 0----\r\n", tuple);
		*ci_plus_enabled = 0;
	} else if ((flag == '*') && (number >= 1)) {
		/* CI+ "v1" is supported */
		*ci_plus_enabled = 1;
		dprintk("--tuple[%s]=set 1----\r\n", tuple);
	} else if ((flag == '+') && (number == 1)) {
		/* CI+ "v1" is supported */
		*ci_plus_enabled = 1;
		dprintk("--tuple[%s]=set 1----\r\n", tuple);
 ...
 */
static void parse_ciprof_compatibility(char *tuple, int len, unsigned int *ci_plus_profile)
{
	int valid;
	int number;
	int i;
...
static int dvb_ca_parse_ciinfo(struct dvb_ca_private *ca, char *tuple, int len)
{
	...
	if (!ciplus_str)
		dprintk("ciplus_str  error  line[%d]\r\n", __LINE__);
	else
		parse_ciplus_compatibility(ciplus_str + 7,
		len - (ciplus_str + 7 - tuple), &dvb_ca_ciplus_enable);

	ciplus_str = findstr((char *)tuple, len, "ciprof=", 7);
	if (!ciplus_str)
		dprintk("ciprof str  error  line[%d]\r\n", __LINE__);
	else
		parse_ciprof_compatibility(ciplus_str + 7,
		len - (ciplus_str + 7 - tuple), &dvb_ca_ci_profire);
...
static int dvb_ca_en50221_parse_attributes(struct dvb_ca_private *ca, int slot)
{
	...
	// CISTPL_DEVICE_0A
	status = dvb_ca_en50221_read_tuple(ca, slot, &address, &tupleType, &tupleLength, tuple);
	if (status < 0) {
		dprintk("read tuple error\r\n");
		return status;
	}
	if (tupleType != 0x1D) {
		dprintk("read tupleType error\r\n");
		return -EINVAL;
	}
	...
	status = dvb_ca_en50221_read_tuple(ca, slot, &address, &tupleType, &tupleLength, tuple);
	if (status < 0) {
		dprintk("read tupleType error line[%d]\r\n", __LINE__);
		return status;
	}
	...
	dprintk("dvb ci parse ci  line[%d]\r\n", __LINE__);
	...
	if (strncmp(dvb_str + 8, "1.00", 4)) {
		dprintk("dvb_ca adapter %d: Unsupported DVB CAM module version %c%c%c%c\n",
	...
	while ((!end_chain) && (address < 0x1000)) {
		...
		case 0x1B:	// CISTPL_CFTABLE_ENTRY
			if (tupleLength < (2 + 11 + 17)) {
				dprintk("dvb ci error 0x1b error  line[%d]\r\n", __LINE__);
				break;
			}
			...
			if (!findstr((char *)tuple, tupleLength, "DVB_HOST", 8) ||
			    !findstr((char *)tuple, tupleLength, "DVB_CI_MODULE", 13))
				break;
...
static int dvb_ca_en50221_read_data(struct dvb_ca_private *ca, int slot, u8 *ebuf, int ecount)
{
	...
	status = ca->pub->read_cam_control(ca->pub, slot, CTRLIF_SIZE_HIGH);
	if (status < 0)
		goto exit;
	bytes_read = status << 8;
	status = ca->pub->read_cam_control(ca->pub, slot, CTRLIF_SIZE_LOW);
	if (status < 0)
		goto exit;
	bytes_read |= status;
	...
	if (!ebuf) {
		if (bytes_read > ca->slot_info[slot].link_buf_size) {
			dprintk("adapter %d:send a buffer larger link buffer size (%i > %i)!\n",
		...
		if (bytes_read < 2) {
			dprintk("dvb_ca adapter %d: CAM sent buffer that was less than 2 bytes!\n",
		...
		if (bytes_read > ecount) {
			dprintk("dvb_ca adapter %d:send buffer larger ecount size!\n",
			ca->dvbdev->adapter->num);
			status = -EIO;
			goto exit;
		}
	...
	for (i = 0; i < bytes_read; i++) {
		/* read byte and check */
		status = ca->pub->read_cam_control(ca->pub, slot, CTRLIF_DATA);
		if (status < 0)
			goto exit;
...
static int dvb_ca_en50221_write_data(struct dvb_ca_private *ca, int slot, u8 *buf, int bytes_write)
{
	...
	status = ca->pub->write_cam_control(ca->pub, slot, CTRLIF_SIZE_HIGH, bytes_write >> 8);
	if (status != 0)
		goto exit;
	...
	for (i = 0; i < bytes_write; i++) {
		status = ca->pub->write_cam_control(ca->pub, slot, CTRLIF_DATA, buf[i]);
		if (status != 0)
			goto exit;
	}
 ...
 */
void dvb_ca_en50221_cimcu_camchange_irq(struct dvb_ca_en50221_cimcu *pubca,
...
static void dvb_ci_slotstate_validate(struct dvb_ca_private *ca, int slot)
{
	int status = 0;

	if (ca_slotstate_validate_t) {
		if (dvb_ca_en50221_parse_attributes(ca, slot) != 0) {
			/* we need this extra check for annoying
			 * interfaces like the budget-av
			 */
			if (!(ca->flags & DVB_CA_EN50221_FLAG_IRQ_CAMCHANGE) &&
			...
			dprintk("dvb_ca adapter %d: Invalid PC card inserted :(\n",
				ca->dvbdev->adapter->num);
		...
		if (dvb_ca_en50221_set_configoption(ca, slot) != 0) {
			dprintk("dvb_ca adapter %d: Unable to initialise CAM :(\n",
			ca->dvbdev->adapter->num);
	...
	if (ca->pub->write_cam_control(ca->pub, slot,
		...
		dprintk("dvb_ca adapter %d: Unable to reset CAM IF\n",
		       ca->dvbdev->adapter->num);
		ca->slot_info[slot].slot_state = DVB_CA_SLOTSTATE_INVALID;
		dvb_ca_en50221_thread_update_delay(ca);
		return;
...
static void dvb_ci_slotstate_linkinit(struct dvb_ca_private *ca, int slot)
{
	int status = 0;
	void *rxbuf = NULL;
	...
	if (dvb_ca_en50221_link_init(ca, slot) != 0) {
		...
		dprintk("dvb_ca adapter %d: DVB CAM link initialisation failed :(\n",
			ca->dvbdev->adapter->num);
		ca->slot_info[slot].slot_state = DVB_CA_SLOTSTATE_INVALID;
		dvb_ca_en50221_thread_update_delay(ca);
		return;
	...
	if (!ca->slot_info[slot].rx_buffer.data) {
		...
		if (!rxbuf) {
			dprintk("dvb_ca adapter %d: Unable to allocate CAM rx buffer :(\n",
				ca->dvbdev->adapter->num);
			ca->slot_info[slot].slot_state = DVB_CA_SLOTSTATE_INVALID;
			dvb_ca_en50221_thread_update_delay(ca);
			return;
	...
	dvb_ca_en50221_thread_update_delay(ca);
	dprintk("dvb_ca adapter %d: DVB CAM detected and initialised successfully\n",
		ca->dvbdev->adapter->num);
}
		...
		for (slot = 0; slot < ca->slot_count; slot++) {
			...
			case DVB_CA_SLOTSTATE_WAITREADY:
				if (time_after(jiffies, ca->slot_info[slot].timeout)) {
					dprintk("dvb_ca adaptor %d: PC card did not respond :(\n",
					       ca->dvbdev->adapter->num);
					ca->slot_info[slot].slot_state = DVB_CA_SLOTSTATE_INVALID;
					dvb_ca_en50221_thread_update_delay(ca);
					break;
			...
			case DVB_CA_SLOTSTATE_VALIDATE:
				dvb_ci_slotstate_validate(ca, slot);
			break;

			case DVB_CA_SLOTSTATE_WAITFR:
				if (time_after(jiffies, ca->slot_info[slot].timeout)) {
					dprintk("dvb_ca adapter %d: DVB CAM did not respond :(\n",
					       ca->dvbdev->adapter->num);
					ca->slot_info[slot].slot_state = DVB_CA_SLOTSTATE_INVALID;
					dvb_ca_en50221_thread_update_delay(ca);
					break;
			...
			case DVB_CA_SLOTSTATE_LINKINIT:
				dvb_ci_slotstate_linkinit(ca, slot);
			break;
			...
			case DVB_CA_SLOTSTATE_RUNNING:
				...
				while ((status = dvb_ca_en50221_read_data(ca, slot, NULL, 0)) > 0) {
					...
					if (++pktcount >= MAX_RX_PACKETS_PER_ITERATION) {
						/* dont sleep; there is likely
						 * to be more data to read
						 */
						ca->wakeup = 1;
						break;
					}
				      ...
				      unsigned int cmd, void *parg)
{
	...
	case CA_GET_SLOT_INFO: {
		...
		info->flags = 0;
		if (cam_state != 0 &&
			ca->slot_info[info->num].slot_state != DVB_CA_SLOTSTATE_NONE &&
			ca->slot_info[info->num].slot_state != DVB_CA_SLOTSTATE_INVALID)
			info->flags = CA_CI_MODULE_PRESENT;

		if (cam_state != 0 && cam_state != 1 &&
				      ...
				      size_t count, loff_t *ppos)
{
	...
	do {
		if (idx == -1) {
			dprintk("adapter %d:read packet end before last_fragment encountered\n",
				ca->dvbdev->adapter->num);
			status = -EIO;
			goto exit;
		}
		...
		if (hdr[0] == connection_id) {
			if (pktlen < count) {
				if ((pktlen + fraglen - offset) > (count - 2)) {
					...
					ca->slot_info[slot].rx_offset += fraglen;
					/* more data for user,
					 * but cannot send,
...
static int dvb_ca_en50221_io_open(struct inode *inode, struct file *file)
{
	...
	for (i = 0; i < ca->slot_count; i++) {
		if (ca->slot_info[i].slot_state == DVB_CA_SLOTSTATE_RUNNING) {
			if (ca->slot_info[i].rx_buffer.data)
			...
			struct dvb_ca_en50221_cimcu *pubca, int flags, int slot_count)
{
	...
	if (IS_ERR(ca->thread)) {
		...
		dprintk("dvb_ca_init: failed to start kernel_thread (%d)\n",
			ret);
		goto unregister_device;
	}
	return 0;
