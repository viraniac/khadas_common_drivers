...
static int aml_tdes_crypt(struct skcipher_request *req, unsigned long mode)
{
	struct aml_tdes_ctx *ctx =
		crypto_skcipher_ctx(crypto_skcipher_reqtfm(req));
	struct aml_tdes_reqctx *rctx = skcipher_request_ctx(req);
	struct aml_tdes_dev *dd;
	int err;
...
static int aml_des_setkey(struct crypto_skcipher *tfm, const u8 *key,
			  u32 keylen)
{
	int err;
	struct crypto_tfm *ctfm = crypto_skcipher_tfm(tfm);
	struct aml_tdes_ctx *ctx = crypto_skcipher_ctx(tfm);
	...
	if (err != 0  && (ctfm->crt_flags & CRYPTO_TFM_REQ_FORBID_WEAK_KEYS)) {
		ctfm->crt_flags |= CRYPTO_TFM_REQ_FORBID_WEAK_KEYS;
		return -EINVAL;
	}
...
static int aml_des_kl_setkey(struct crypto_skcipher *tfm, const u8 *key,
			     u32 keylen)
{
	struct aml_tdes_ctx *ctx = crypto_skcipher_ctx(tfm);
...
static int aml_tdes_ecb_encrypt(struct skcipher_request *req)
{
	return aml_tdes_crypt(req,
			TDES_FLAGS_ENCRYPT);
}

static int aml_tdes_ecb_decrypt(struct skcipher_request *req)
{
	return aml_tdes_crypt(req,
			0);
}

static int aml_tdes_cbc_encrypt(struct skcipher_request *req)
{
	return aml_tdes_crypt(req,
			TDES_FLAGS_ENCRYPT | TDES_FLAGS_CBC);
}

static int aml_tdes_cbc_decrypt(struct skcipher_request *req)
{
	return aml_tdes_crypt(req,
			TDES_FLAGS_CBC);
}

static int aml_tdes_lite_init_tfm(struct crypto_skcipher *tfm);
static void aml_tdes_lite_exit_tfm(struct crypto_skcipher *tfm);
static int aml_tdes_lite_setkey(struct crypto_skcipher *tfm, const u8 *key,
				unsigned int keylen);
static int aml_tdes_kl_setkey(struct crypto_skcipher *tfm, const u8 *key,
			      unsigned int keylen);
static struct skcipher_alg des_tdes_algs[] = {
	{
		.base.cra_name        = "ecb(des-aml)",
		.base.cra_driver_name = "ecb-des-aml",
		.base.cra_priority  =  100,
		.base.cra_flags     =  CRYPTO_ALG_ASYNC,
		.base.cra_blocksize =  DES_BLOCK_SIZE,
		.base.cra_ctxsize   =  sizeof(struct aml_tdes_ctx),
		.base.cra_alignmask =  0,
		.base.cra_module    =  THIS_MODULE,

		.init      =  aml_tdes_lite_init_tfm,
		.exit      =  aml_tdes_lite_exit_tfm,
	...
	{
		.base.cra_name        =  "cbc(des-aml)",
		.base.cra_driver_name =  "cbc-des-aml",
		.base.cra_priority  =  100,
		.base.cra_flags     =  CRYPTO_ALG_ASYNC,
		.base.cra_blocksize =  DES_BLOCK_SIZE,
		.base.cra_ctxsize   =  sizeof(struct aml_tdes_ctx),
	...
	{
		.base.cra_name        = "ecb(des3_ede-aml)",
		.base.cra_driver_name = "ecb-tdes-aml",
		.base.cra_priority   = 100,
	...
	{
		.base.cra_name        = "cbc(des3_ede-aml)",
		.base.cra_driver_name = "cbc-tdes-aml",
		.base.cra_priority  = 100,
		.base.cra_flags      = CRYPTO_ALG_ASYNC | CRYPTO_ALG_NEED_FALLBACK,
		.base.cra_blocksize = DES_BLOCK_SIZE,
		.base.cra_ctxsize   = sizeof(struct aml_tdes_ctx),
	...
	{
		.base.cra_name        = "ecb(des-kl-aml)",
		.base.cra_driver_name = "ecb-des-kl-aml",
		.base.cra_priority  =  100,
		.base.cra_flags     =  CRYPTO_ALG_ASYNC,
		.base.cra_blocksize =  DES_BLOCK_SIZE,
		.base.cra_ctxsize   =  sizeof(struct aml_tdes_ctx),
	...
	{
		.base.cra_name        =  "cbc(des-kl-aml)",
		.base.cra_driver_name =  "cbc-des-kl-aml",
		.base.cra_priority  =  100,
		.base.cra_flags     =  CRYPTO_ALG_ASYNC,
		.base.cra_blocksize =  DES_BLOCK_SIZE,
		.base.cra_ctxsize   =  sizeof(struct aml_tdes_ctx),
	...
	{
		.base.cra_name        = "ecb(tdes-kl-aml)",
		.base.cra_driver_name = "ecb-tdes-kl-aml",
		.base.cra_priority   = 100,
		.base.cra_flags      = CRYPTO_ALG_ASYNC,
		.base.cra_blocksize  = DES_BLOCK_SIZE,
		.base.cra_ctxsize    = sizeof(struct aml_tdes_ctx),
	...
	{
		.base.cra_name        = "cbc(tdes-kl-aml)",
		.base.cra_driver_name = "cbc-tdes-kl-aml",
		.base.cra_priority  = 100,
		.base.cra_flags     = CRYPTO_ALG_ASYNC,
		.base.cra_blocksize = DES_BLOCK_SIZE,
		.base.cra_ctxsize   = sizeof(struct aml_tdes_ctx),
...
static int aml_tdes_lite_init_tfm(struct crypto_skcipher *tfm)
{
	...
	crypto_skcipher_set_reqsize(tfm, sizeof(struct aml_tdes_reqctx));
...
static struct skcipher_alg tdes_lite_algs[] = {
	{
		.base.cra_name        = "ecb(des3_ede-aml)",
		.base.cra_driver_name = "ecb-tdes-lite-aml",
		.base.cra_priority   = 100,
		.base.cra_flags      = CRYPTO_ALG_ASYNC |  CRYPTO_ALG_NEED_FALLBACK,
		.base.cra_blocksize  = DES_BLOCK_SIZE,
		.base.cra_ctxsize    = sizeof(struct aml_tdes_ctx),
	...
	{
		.base.cra_name        = "cbc(des3_ede-aml)",
		.base.cra_driver_name = "cbc-tdes-lite-aml",
		.base.cra_priority  = 100,
		.base.cra_flags     = CRYPTO_ALG_ASYNC | CRYPTO_ALG_NEED_FALLBACK,
		.base.cra_blocksize = DES_BLOCK_SIZE,
		.base.cra_ctxsize   = sizeof(struct aml_tdes_ctx),
				     ...
				     struct aml_tdes_info *tdes_info)
{
	...
	for (i = 0; i < tdes_info->num_algs; i++)
		crypto_unregister_skcipher(&tdes_info->algs[i]);
}
				  ...
				  struct aml_tdes_info *tdes_info)
{
	...
	for (i = 0; i < tdes_info->num_algs; i++) {
		err = crypto_register_skcipher(&tdes_info->algs[i]);
		if (err)
			goto err_tdes_algs;
	}
...
err_tdes_algs:
	for (j = 0; j < i; j++)
		crypto_unregister_skcipher(&tdes_info->algs[j]);
