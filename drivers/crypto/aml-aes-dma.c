	...
	if (dd->flags & AES_FLAGS_DMA) {
		...
		if  (dd->flags & AES_FLAGS_FAST) {
			...
			if (dd->flags & AES_FLAGS_CBC) {
				...
				aml_aes_ctr_update_iv(dd->req->iv,
						      dma_nblock);
			}
		} else {
...
static int aml_aes_crypt(struct skcipher_request *req, unsigned long mode)
{
	struct aml_aes_ctx *ctx =
		crypto_skcipher_ctx(crypto_skcipher_reqtfm(req));
	struct aml_aes_reqctx *rctx = skcipher_request_ctx(req);
	struct aml_aes_dev *dd;
	int ret;
...
static int aml_aes_kl_setkey(struct crypto_skcipher *tfm, const u8 *key,
			     unsigned int keylen)
{
	struct aml_aes_ctx *ctx = crypto_skcipher_ctx(tfm);
	int ret = 0;
...
static int aml_aes_lite_setkey(struct crypto_skcipher *tfm, const u8 *key,
			       unsigned int keylen)
{
	struct aml_aes_ctx *ctx = crypto_skcipher_ctx(tfm);
	int ret = 0;
...
static int aml_aes_ecb_encrypt(struct skcipher_request *req)
{
	return aml_aes_crypt(req,
			AES_FLAGS_ENCRYPT);
}

static int aml_aes_ecb_decrypt(struct skcipher_request *req)
{
	return aml_aes_crypt(req,
			0);
}

static int aml_aes_cbc_encrypt(struct skcipher_request *req)
{
	return aml_aes_crypt(req,
			AES_FLAGS_ENCRYPT | AES_FLAGS_CBC);
}

static int aml_aes_cbc_decrypt(struct skcipher_request *req)
{
	return aml_aes_crypt(req,
			AES_FLAGS_CBC);
}

static int aml_aes_ctr_encrypt(struct skcipher_request *req)
{
	return aml_aes_crypt(req,
			AES_FLAGS_ENCRYPT | AES_FLAGS_CTR);
}

static int aml_aes_ctr_decrypt(struct skcipher_request *req)
{
...
static int aml_aes_init_tfm(struct crypto_skcipher *tfm)
{
	struct aml_aes_ctx *ctx = crypto_skcipher_ctx(tfm);

	crypto_skcipher_set_reqsize(tfm, sizeof(struct aml_aes_reqctx));
	ctx->fallback = NULL;

	return 0;
}

static void aml_aes_exit_tfm(struct crypto_skcipher *tfm)
{
}

static int aml_aes_setkey(struct crypto_skcipher *tfm, const u8 *key,
			  unsigned int keylen)
{
	struct aml_aes_ctx *ctx = crypto_skcipher_ctx(tfm);
...
static struct skcipher_alg aes_algs[] = {
	{
		.base.cra_name         = "ecb(aes-aml)",
		.base.cra_driver_name  = "ecb-aes-aml",
		.base.cra_priority   = 100,
		.base.cra_flags      = CRYPTO_ALG_ASYNC,
		.base.cra_blocksize  = AES_BLOCK_SIZE,
		.base.cra_ctxsize    = sizeof(struct aml_aes_ctx),
		.base.cra_alignmask  = 0xf,
		.base.cra_module     = THIS_MODULE,

		.init       = aml_aes_init_tfm,
		.exit       = aml_aes_exit_tfm,
	...
	{
		.base.cra_name         = "cbc(aes-aml)",
		.base.cra_driver_name  = "cbc-aes-aml",
		.base.cra_priority   = 100,
		.base.cra_flags      = CRYPTO_ALG_ASYNC,
		.base.cra_blocksize  = AES_BLOCK_SIZE,
		.base.cra_ctxsize    = sizeof(struct aml_aes_ctx),
	...
	{
		.base.cra_name        = "ctr(aes-aml)",
		.base.cra_driver_name = "ctr-aes-aml",
		.base.cra_priority    = 100,
		.base.cra_flags      = CRYPTO_ALG_ASYNC,
		.base.cra_blocksize  = AES_BLOCK_SIZE,
		.base.cra_ctxsize    = sizeof(struct aml_aes_ctx),
...
static int aml_aes_lite_init_tfm(struct crypto_skcipher *tfm)
{
	struct aml_aes_ctx *ctx = crypto_skcipher_ctx(tfm);
	const char *alg_name = crypto_tfm_alg_name(&tfm->base);
	const char *driver_name = crypto_tfm_alg_driver_name(&tfm->base);
...
static void aml_aes_lite_exit_tfm(struct crypto_skcipher *tfm)
{
	struct aml_aes_ctx *ctx = crypto_skcipher_ctx(tfm);
...
static struct skcipher_alg aes_lite_algs[] = {
	{
		.base.cra_name         = "ecb(aes-aml)",
		.base.cra_driver_name  = "ecb-aes-lite-aml",
		.base.cra_priority   = 100,
		...
		.init       = aml_aes_lite_init_tfm,
		.exit       = aml_aes_lite_exit_tfm,
		.min_keysize	=    AES_MIN_KEY_SIZE,
		.max_keysize	=    AES_MAX_KEY_SIZE,
	...
	{
		.base.cra_name         = "cbc(aes-aml)",
		.base.cra_driver_name  = "cbc-aes-lite-aml",
		.base.cra_priority   = 100,
		.base.cra_flags      = CRYPTO_ALG_ASYNC | CRYPTO_ALG_NEED_FALLBACK,
		.base.cra_blocksize  = AES_BLOCK_SIZE,
		.base.cra_ctxsize    = sizeof(struct aml_aes_ctx),
	...
	{
		.base.cra_name        = "ctr(aes-aml)",
		.base.cra_driver_name = "ctr-aes-lite-aml",
		.base.cra_priority    = 100,
		.base.cra_flags      = CRYPTO_ALG_ASYNC | CRYPTO_ALG_NEED_FALLBACK,
		.base.cra_blocksize  = AES_BLOCK_SIZE,
		.base.cra_ctxsize    = sizeof(struct aml_aes_ctx),
	...
	{
		.base.cra_name         = "ecb(aes-kl-aml)",
		.base.cra_driver_name  = "ecb-aes-kl-aml",
		.base.cra_priority   = 100,
		.base.cra_flags      = CRYPTO_ALG_ASYNC | CRYPTO_ALG_NEED_FALLBACK,
		.base.cra_blocksize  = AES_BLOCK_SIZE,
		.base.cra_ctxsize    = sizeof(struct aml_aes_ctx),
	...
	{
		.base.cra_name         = "cbc(aes-kl-aml)",
		.base.cra_driver_name  = "cbc-aes-kl-aml",
		.base.cra_priority   = 100,
		.base.cra_flags      = CRYPTO_ALG_ASYNC | CRYPTO_ALG_NEED_FALLBACK,
		.base.cra_blocksize  = AES_BLOCK_SIZE,
		.base.cra_ctxsize    = sizeof(struct aml_aes_ctx),
	...
	{
		.base.cra_name         = "ctr(aes-kl-aml)",
		.base.cra_driver_name  = "ctr-aes-kl-aml",
		.base.cra_priority    = 100,
		.base.cra_flags      = CRYPTO_ALG_ASYNC | CRYPTO_ALG_NEED_FALLBACK,
		.base.cra_blocksize  = AES_BLOCK_SIZE,
		.base.cra_ctxsize    = sizeof(struct aml_aes_ctx),
...
static irqreturn_t aml_aes_irq(int irq, void *dev_id)
{
	...
	u32 status = aml_read_crypto_reg(aes_dd->status);
	...
	if (status) {
		...
		if ((aes_dd->flags & AES_FLAGS_DMA) &&
			...
			aml_write_crypto_reg(aes_dd->status, 0xff);
			aes_dd->dma->dma_busy &= ~DMA_FLAG_AES_IN_USE;
				    ...
				    const struct aml_aes_info *aes_info)
{
	...
	for (i = 0; i < aes_info->num_algs; i++)
		crypto_unregister_skcipher(&aes_info->algs[i]);
}
				 ...
				 const struct aml_aes_info *aes_info)
{
	...
	for (i = 0; i < aes_info->num_algs; i++) {
		err = crypto_register_skcipher(&aes_info->algs[i]);
		if (err)
			goto err_aes_algs;
	}
...
err_aes_algs:
	for (j = 0; j < i; j++)
		crypto_unregister_skcipher(&aes_info->algs[j]);
