...
{\
	...
	if (src != -1 && aml_dvb_device.dmx[i].id != -1) {\
...
{\
	...
	if (dump_ts_select != -1 && aml_dvb_device.dmx[i].id != -1) {\
...
{\
	...
	if (src != -1 && aml_dvb_device.asyncfifo[i].id != -1) {\
...
{\
	...
	if (ret == 0)\
		fsize = value;\
	if (fsize != afifo->flush_size && aml_dvb_device.asyncfifo[i].id != -1) {\
