...
static void dwc3_otg_disable_events(struct aml_dwc3 *dwc, u32 disable_mask)
{
...
static void dwc3_otg_enable_events(struct aml_dwc3 *dwc, u32 enable_mask)
{
	u32 reg = dwc3_readl(dwc->regs, DWC3_OEVTEN);
...
static void dwc3_otg_clear_events(struct aml_dwc3 *dwc)
{
...
static irqreturn_t dwc3_otg_thread_irq(int irq, void *_dwc)
{
	...
	aml_dwc3_set_mode(dwc, DWC3_GCTL_PRTCAP_OTG);
...
static void dwc3_otgregs_init(struct aml_dwc3 *dwc)
{
	u32 reg;
...
static int dwc3_otg_get_irq(struct aml_dwc3 *dwc)
{
...
void aml_dwc3_dwc3_otg_init(struct aml_dwc3 *dwc)
{
	...
	aml_dwc3_set_prtcap(dwc, DWC3_GCTL_PRTCAP_OTG);
...
static void dwc3_otg_host_exit(struct aml_dwc3 *dwc)
{
	u32 reg;
...
static void dwc3_otg_device_init(struct aml_dwc3 *dwc)
{
	u32 reg;
...
static void dwc3_otg_device_exit(struct aml_dwc3 *dwc)
{
	u32 reg;
...
static void dwc3_drd_update(struct aml_dwc3 *dwc)
{
	int id;
...
{
	struct aml_dwc3 *dwc = container_of(nb, struct aml_dwc3, edev_nb);

	aml_dwc3_set_mode(dwc, event ?
		      DWC3_GCTL_PRTCAP_HOST :
		      DWC3_GCTL_PRTCAP_DEVICE);
...
static struct extcon_dev *dwc3_get_extcon(struct aml_dwc3 *dwc)
{
...
static int dwc3_usb_role_switch_set(struct usb_role_switch *sw,
				    enum usb_role role)
{
	struct aml_dwc3 *dwc = usb_role_switch_get_drvdata(sw);
	u32 mode;
...
static int dwc3_setup_role_switch(struct aml_dwc3 *dwc)
{
