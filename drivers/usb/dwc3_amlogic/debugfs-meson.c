		...
		const char __user *ubuf, size_t count, loff_t *ppos)
{
	...
	spin_lock_irqsave(&dwc->lock, flags);
	aml_dwc3_gadget_set_test_mode(dwc, testmode);
	spin_unlock_irqrestore(&dwc->lock, flags);
		...
		const char __user *ubuf, size_t count, loff_t *ppos)
{
	...
	struct aml_dwc3		*dwc = s->private;
	unsigned long		flags;
	enum aml_dwc3_link_state	state = 0;
	char			buf[32];
	u32			reg;
	u8			speed;
	int			ret;
	...
	aml_dwc3_gadget_set_link_state(dwc, state);
	spin_unlock_irqrestore(&dwc->lock, flags);
...
{
	struct aml_dwc3_ep		*dep = s->private;
	...
	spin_lock_irqsave(&dwc->lock, flags);
	val = aml_dwc3_core_fifo_space(dep, DWC3_TXFIFO);
...
static int dwc3_rx_fifo_size_show(struct seq_file *s, void *unused)
{
	...
	spin_lock_irqsave(&dwc->lock, flags);
	val = aml_dwc3_core_fifo_space(dep, DWC3_RXFIFO);
...
static int dwc3_tx_request_queue_show(struct seq_file *s, void *unused)
{
	...
	spin_lock_irqsave(&dwc->lock, flags);
	val = aml_dwc3_core_fifo_space(dep, DWC3_TXREQQ);
	seq_printf(s, "%u\n", val);
	spin_unlock_irqrestore(&dwc->lock, flags);
...
static int dwc3_rx_request_queue_show(struct seq_file *s, void *unused)
{
	...
	spin_lock_irqsave(&dwc->lock, flags);
	val = aml_dwc3_core_fifo_space(dep, DWC3_RXREQQ);
	seq_printf(s, "%u\n", val);
	spin_unlock_irqrestore(&dwc->lock, flags);
...
static int dwc3_rx_info_queue_show(struct seq_file *s, void *unused)
{
	...
	spin_lock_irqsave(&dwc->lock, flags);
	val = aml_dwc3_core_fifo_space(dep, DWC3_RXINFOQ);
	seq_printf(s, "%u\n", val);
	spin_unlock_irqrestore(&dwc->lock, flags);
...
static int dwc3_descriptor_fetch_queue_show(struct seq_file *s, void *unused)
{
	...
	spin_lock_irqsave(&dwc->lock, flags);
	val = aml_dwc3_core_fifo_space(dep, DWC3_DESCFETCHQ);
	seq_printf(s, "%u\n", val);
	spin_unlock_irqrestore(&dwc->lock, flags);
...
static int dwc3_event_queue_show(struct seq_file *s, void *unused)
{
	...
	spin_lock_irqsave(&dwc->lock, flags);
	val = aml_dwc3_core_fifo_space(dep, DWC3_EVENTQ);
	seq_printf(s, "%u\n", val);
	spin_unlock_irqrestore(&dwc->lock, flags);
...
static int dwc3_trb_ring_show(struct seq_file *s, void *unused)
{
	...
	for (i = 0; i < DWC3_TRB_NUM; i++) {
		struct aml_dwc3_trb *trb = &dep->trb_pool[i];
...
static const struct aml_dwc3_ep_file_map dwc3_ep_file_map[] = {
...
void aml_dwc3_debugfs_create_endpoint_dir(struct aml_dwc3_ep *dep)
{
	struct dentry		*dir;
	int			i;
...
void aml_dwc3_debugfs_remove_endpoint_dir(struct aml_dwc3_ep *dep)
{
	debugfs_lookup_and_remove(dep->name, dep->dwc->debug_root);
}

void aml_dwc3_debugfs_init(struct aml_dwc3 *dwc)
{
	struct dentry		*root;
...
void aml_dwc3_debugfs_exit(struct aml_dwc3 *dwc)
{
