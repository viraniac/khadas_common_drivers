...
static const struct aml_xhci_driver_overrides xhci_plat_overrides __initconst = {
	.extra_priv_size = sizeof(struct aml_xhci_plat_priv),
...
{
	struct aml_xhci_plat_priv *priv = aml_hcd_to_xhci_priv(hcd);
...
static void xhci_plat_quirks(struct device *dev, struct aml_xhci_hcd *xhci)
{
	struct aml_xhci_plat_priv *priv = aml_xhci_to_priv(xhci);
...
static int xhci_plat_setup(struct usb_hcd *hcd)
{
	...
	return aml_xhci_gen_setup(hcd, xhci_plat_quirks);
}

static int xhci_plat_start(struct usb_hcd *hcd)
{
	xhci_priv_plat_start(hcd);
	return aml_xhci_run(hcd);
}

#ifdef CONFIG_OF
static const struct aml_xhci_plat_priv xhci_plat_marvell_armada = {
...
static int xhci_plat_probe(struct platform_device *pdev)
{
	...
	int			ret;
	int			irq;
	struct aml_xhci_plat_priv	*priv = NULL;
...
static int __maybe_unused xhci_plat_suspend(struct device *dev)
{
	...
	if (ret)
		return ret;
	/*
	 * aml_xhci_suspend() needs `do_wakeup` to know whether host is allowed
	 * to do wakeup during suspend.
	 */
	ret = aml_xhci_suspend(xhci, device_may_wakeup(dev));
	if (ret)
		return ret;
...
static int __maybe_unused xhci_plat_runtime_suspend(struct device *dev)
{
	...
	return aml_xhci_suspend(xhci, true);
}
