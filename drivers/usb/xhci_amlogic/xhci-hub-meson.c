...
static int xhci_create_usb3x_bos_desc(struct aml_xhci_hcd *xhci, char *buf,
				      u16 wLength)
{
	struct usb_bos_descriptor	*bos;
	struct usb_ss_cap_descriptor	*ss_cap;
	struct usb_ssp_cap_descriptor	*ssp_cap;
	struct aml_xhci_port_cap		*port_cap = NULL;
	u16				bcdUSB;
	u32				reg;
	u32				min_rate = 0;
	u8				min_ssid;
...
static void xhci_common_hub_descriptor(struct aml_xhci_hcd *xhci,
...
static void xhci_usb2_hub_descriptor(struct usb_hcd *hcd, struct aml_xhci_hcd *xhci,
...
static void xhci_usb3_hub_descriptor(struct usb_hcd *hcd, struct aml_xhci_hcd *xhci,
		struct usb_hub_descriptor *desc)
{
	int ports;
	u16 port_removable;
...
static void xhci_hub_descriptor(struct usb_hcd *hcd, struct aml_xhci_hcd *xhci,
		struct usb_hub_descriptor *desc)
{
...
static int xhci_stop_device(struct aml_xhci_hcd *xhci, int slot_id, int suspend)
{
	...
	trace_aml_xhci_stop_device(virt_dev);

	cmd = aml_xhci_alloc_command(xhci, true, GFP_NOIO);
	if (!cmd)
		return -ENOMEM;
	...
	for (i = LAST_EP_INDEX; i > 0; i--) {
		if (virt_dev->eps[i].ring && virt_dev->eps[i].ring->dequeue) {
			...
			ep_ctx = aml_xhci_get_ep_ctx(xhci, virt_dev->out_ctx, i);

			/* Check ep is running, required by AMD SNPS 3.1 xHC */
			if (GET_EP_CTX_STATE(ep_ctx) != EP_STATE_RUNNING)
				continue;

			command = aml_xhci_alloc_command(xhci, false, GFP_NOWAIT);
			if (!command) {
			...
			ret = aml_xhci_queue_stop_endpoint(xhci, command, slot_id,
						       i, suspend);
			if (ret) {
				spin_unlock_irqrestore(&xhci->lock, flags);
	...
	ret = aml_xhci_queue_stop_endpoint(xhci, cmd, slot_id, 0, suspend);
	if (ret) {
		spin_unlock_irqrestore(&xhci->lock, flags);
		goto cmd_cleanup;
	}
	...
	if (cmd->status == COMP_COMMAND_ABORTED ||
		...
		aml_xhci_warn(xhci, "Timeout while waiting for stop endpoint command\n");
		ret = -ETIME;
	}
	...
	for (i = 0; i < LAST_EP_INDEX + 1; i++) {
		...
		if (ep->ep_state & EP_HAS_STREAMS) {
			for (s = 1; s < ep->stream_info->num_streams; s++)
				aml_xhci_ring_ep_doorbell(xhci, slot_id, i, s);
		} else if (ep->ring && ep->ring->dequeue) {
			aml_xhci_ring_ep_doorbell(xhci, slot_id, i, 0);
		}
	}
...
static void xhci_disable_port(struct usb_hcd *hcd, struct aml_xhci_hcd *xhci,
		u16 wIndex, __le32 __iomem *addr, u32 port_status)
{
	/* Don't allow the USB core to disable SuperSpeed ports. */
	if (hcd->speed >= HCD_USB3) {
		aml_xhci_dbg(xhci, "Ignoring request to disable "
				"SuperSpeed port.\n");
		return;
	}
	...
	aml_xhci_dbg(xhci, "disable port %d-%d, portsc: 0x%x\n",
		 hcd->self.busnum, wIndex + 1, port_status);
}

static void xhci_clear_port_change_bit(struct aml_xhci_hcd *xhci, u16 wValue,
		u16 wIndex, __le32 __iomem *addr, u32 port_status)
{
	...
	aml_xhci_dbg(xhci, "clear port%d %s change, portsc: 0x%x\n",
 ...
 * method.
 */
static void xhci_set_port_power(struct aml_xhci_hcd *xhci, struct usb_hcd *hcd,
	...
	__must_hold(&xhci->lock)
{
	...
	aml_xhci_dbg(xhci, "set port power %d-%d %s, portsc: 0x%x\n",
...
static void xhci_port_set_test_mode(struct aml_xhci_hcd *xhci,
	...
	__must_hold(&xhci->lock)
{
	...
	aml_xhci_dbg(xhci, "Disable all slots\n");
	spin_unlock_irqrestore(&xhci->lock, *flags);
	for (i = 1; i <= HCS_MAX_SLOTS(xhci->hcs_params1); i++) {
		if (!xhci->devs[i])
			continue;

		retval = aml_xhci_disable_slot(xhci, i);
		aml_xhci_free_virt_device(xhci, i);
		if (retval)
			aml_xhci_err(xhci, "Failed to disable slot %d, %d. Enter test mode anyway\n",
				 i, retval);
	}
	spin_lock_irqsave(&xhci->lock, *flags);
	/* Put all ports to the Disable state by clear PP */
	aml_xhci_dbg(xhci, "Disable all port (PP = 0)\n");
	...
	/* Stop the controller */
	aml_xhci_dbg(xhci, "Stop controller\n");
	...
	aml_xhci_dbg(xhci, "Enter Test Mode: %d, Port_id=%d\n",
					test_mode, wIndex + 1);
	xhci_port_set_test_mode(xhci, test_mode, wIndex);
	return retval;
}

static int xhci_exit_test_mode(struct aml_xhci_hcd *xhci)
{
	int retval;

	if (!xhci->test_mode) {
		aml_xhci_err(xhci, "Not in test mode, do nothing.\n");
		return 0;
	}
			 ...
			 u32 link_state)
{
	...
	temp = aml_xhci_port_state_to_neutral(portsc);
	temp &= ~PORT_PLS_MASK;
	temp |= PORT_LINK_STROBE | link_state;
	writel(temp, port->addr);

	aml_xhci_dbg(xhci, "Set port %d-%d link state, portsc: 0x%x, write 0x%x",
		 port->rhub->hcd->self.busnum, port->hcd_portnum + 1,
		 portsc, temp);
}

static void xhci_set_remote_wake_mask(struct aml_xhci_hcd *xhci,
				      struct aml_xhci_port *port, u16 wake_mask)
{
	u32 temp;
...
static void xhci_hub_report_usb3_link_state(struct aml_xhci_hcd *xhci,
 ...
 */
static void xhci_del_comp_mod_timer(struct aml_xhci_hcd *xhci, u32 status,
				    u16 wIndex)
{
...
static int xhci_handle_usb2_port_link_resume(struct aml_xhci_port *port,
					     ...
					     unsigned long *flags)
{
	...
	/* Has resume been signalled for USB_RESUME_TIME yet? */
		...
		aml_xhci_dbg(xhci, "resume USB2 port %d-%d\n",
		...
		if (time_left) {
			...
			aml_xhci_warn(xhci, "Port resume timed out, port %d-%d: 0x%x\n",
				  hcd->self.busnum, wIndex + 1, port_status);
...
static void xhci_get_usb3_port_status(struct aml_xhci_port *port, u32 *status,
				      u32 portsc)
{
	struct aml_xhci_bus_state *bus_state;
	struct aml_xhci_hcd	*xhci;
...
static void xhci_get_usb2_port_status(struct aml_xhci_port *port, u32 *status,
		...
		u16 wIndex, char *buf, u16 wLength)
{
	...
	case GetHubDescriptor:
		...
		if (hcd->speed >= HCD_USB3 &&
			...
			aml_xhci_dbg(xhci, "Wrong hub descriptor type for "
					"USB 3.0 roothub.\n");
			goto error;
		}
	...
	case GetPortStatus:
		...
		aml_xhci_dbg(xhci, "Get port status %d-%d read: 0x%x, return 0x%x",
		...
		if (wValue == 0x02) {
			...
			if (hcd->speed < HCD_USB31 || wLength != 8) {
				aml_xhci_err(xhci, "get ext port status invalid parameter\n");
				retval = -EINVAL;
				break;
			}
	...
	case SetPortFeature:
		...
		case USB_PORT_FEAT_SUSPEND:
			...
			if ((temp & PORT_PLS_MASK) != XDEV_U0) {
				/* Resume the port to U0 first */
				aml_xhci_set_link_state(xhci, ports[wIndex],
							XDEV_U0);
				spin_unlock_irqrestore(&xhci->lock, flags);
				msleep(10);
				spin_lock_irqsave(&xhci->lock, flags);
			...
			if ((temp & PORT_PE) == 0 || (temp & PORT_RESET)
				|| (temp & PORT_PLS_MASK) >= XDEV_U3) {
				aml_xhci_warn(xhci, "USB core suspending port %d-%d not in U0/U1/U2\n",
					  hcd->self.busnum, wIndex + 1);
				goto error;
			}
		...
		case USB_PORT_FEAT_LINK_STATE:
			...
			if (link_state == USB_SS_PORT_LS_SS_DISABLED) {
				aml_xhci_dbg(xhci, "Disable port %d-%d\n",
					 hcd->self.busnum, wIndex + 1);
				temp = aml_xhci_port_state_to_neutral(temp);
				/*
			...
			if (link_state == USB_SS_PORT_LS_RX_DETECT) {
				aml_xhci_dbg(xhci, "Enable port %d-%d\n",
					 hcd->self.busnum, wIndex + 1);
				aml_xhci_set_link_state(xhci, ports[wIndex],
							link_state);
				temp = readl(ports[wIndex]->addr);
			...
			if (link_state == USB_SS_PORT_LS_COMP_MOD) {
				if (!HCC2_CTC(xhci->hcc_params2)) {
					aml_xhci_dbg(xhci, "CTC flag is 0, port already supports entering compliance mode\n");
					break;
				}

				if ((temp & PORT_CONNECT)) {
					aml_xhci_warn(xhci, "Can't set compliance mode when port is connected\n");
					goto error;
				}

				aml_xhci_dbg(xhci, "Enable compliance mode transition for port %d-%d\n",
					 hcd->self.busnum, wIndex + 1);
				aml_xhci_set_link_state(xhci, ports[wIndex],
						link_state);
			...
			if (link_state > USB_SS_PORT_LS_U3) {
				aml_xhci_warn(xhci, "Cannot set port %d-%d link state %d\n",
					  hcd->self.busnum, wIndex + 1,
					  link_state);
				goto error;
			}
			...
			if (link_state == USB_SS_PORT_LS_U0) {
				...
				if (!wait_for_completion_timeout(&bus_state->u3exit_done[wIndex],
					...
					aml_xhci_dbg(xhci, "missing U0 port change event for port %d-%d\n",
						 hcd->self.busnum, wIndex + 1);
				spin_lock_irqsave(&xhci->lock, flags);
				temp = readl(ports[wIndex]->addr);
				break;
		...
		case USB_PORT_FEAT_RESET:
			...
			temp = readl(ports[wIndex]->addr);
			aml_xhci_dbg(xhci, "set port reset, actual port %d-%d status  = 0x%x\n",
		...
		case USB_PORT_FEAT_REMOTE_WAKE_MASK:
			xhci_set_remote_wake_mask(xhci, ports[wIndex],
						  wake_mask);
			temp = readl(ports[wIndex]->addr);
			aml_xhci_dbg(xhci, "set port remote wake mask, actual port %d-%d status  = 0x%x\n",
				 hcd->self.busnum, wIndex + 1, temp);
			break;
...
int aml_xhci_bus_suspend(struct usb_hcd *hcd)
{
	...
	if (wake_enabled) {
		if (bus_state->resuming_ports ||	/* USB2 */
			...
			spin_unlock_irqrestore(&xhci->lock, flags);
			aml_xhci_dbg(xhci, "usb%d bus suspend to fail because a port is resuming\n",
				 hcd->self.busnum);
			return -EBUSY;
		}
	}
...
retry:
		...
		t2 = aml_xhci_port_state_to_neutral(t1);
		...
		if ((hcd->speed >= HCD_USB3) && retries-- &&
			...
			spin_lock_irqsave(&xhci->lock, flags);
			aml_xhci_dbg(xhci, "port %d-%d polling in bus suspend, waiting\n",
		...
		if (t1 & PORT_OC) {
			bus_state->bus_suspended = 0;
			spin_unlock_irqrestore(&xhci->lock, flags);
			aml_xhci_dbg(xhci, "Bus suspend bailout, port over-current detected\n");
			return -EBUSY;
		}
		...
		if ((t1 & PORT_PE) && (t1 & PORT_PLS_MASK) == XDEV_U0) {
			if ((t1 & PORT_CSC) && wake_enabled) {
				bus_state->bus_suspended = 0;
				spin_unlock_irqrestore(&xhci->lock, flags);
				aml_xhci_dbg(xhci, "Bus suspend bailout, port connect change\n");
				return -EBUSY;
			}
			aml_xhci_dbg(xhci, "port %d-%d not suspended\n",
				 hcd->self.busnum, port_index + 1);
			t2 &= ~PORT_PLS_MASK;
		...
		if (wake_enabled) {
			...
			if ((xhci->quirks & XHCI_U2_DISABLE_WAKE) &&
			    (hcd->speed < HCD_USB3)) {
				if (aml_usb_amd_pt_check_port(hcd->self.controller,
							  port_index))
					t2 &= ~PORT_WAKE_BITS;
			}
		} else
		...
		t1 = aml_xhci_port_state_to_neutral(t1);
		if (t1 != t2)
 ...
 */
static bool xhci_port_missing_cas_quirk(struct aml_xhci_port *port)
{
	u32 portsc;
...
int aml_xhci_bus_resume(struct usb_hcd *hcd)
{
	...
	while (port_index--) {
		...
		if ((xhci->quirks & XHCI_MISSING_CAS) &&
			...
			aml_xhci_dbg(xhci, "reset stuck port %d-%d\n",
				 hcd->self.busnum, port_index + 1);
		...
		if (test_bit(port_index, &bus_state->bus_suspended))
			...
			case XDEV_U3:
				portsc = aml_xhci_port_state_to_neutral(portsc);
	...
	if (hcd->speed < HCD_USB3) {
		...
		for_each_set_bit(port_index, &bus_state->bus_suspended,
			...
			aml_xhci_test_and_clear_bit(xhci, ports[port_index],
						PORT_PLC);
			aml_xhci_set_link_state(xhci, ports[port_index], XDEV_U0);
		}
	}
...
{
	struct aml_xhci_hub *rhub = aml_xhci_get_rhub(hcd);
