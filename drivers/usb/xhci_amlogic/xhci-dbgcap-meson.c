...
static void dbc_free_ctx(struct device *dev, struct aml_xhci_container_ctx *ctx)
{
	if (!ctx)
		return;
...
static void dbc_ring_free(struct device *dev, struct aml_xhci_ring *ring)
{
	if (!ring)
		return;
...
static u32 xhci_dbc_populate_strings(struct aml_dbc_str_descs *strings)
{
...
static void xhci_dbc_init_contexts(struct aml_xhci_dbc *dbc, u32 string_length)
{
	struct aml_dbc_info_context	*info;
	...
	info			= (struct aml_dbc_info_context *)dbc->ctx->bytes;
	dma			= dbc->string_dma;
	...
	ep_ctx			= aml_dbc_bulkout_ctx(dbc);
	max_burst		= DBC_CTRL_MAXBURST(readl(&dbc->regs->control));
	deq			= aml_dbc_bulkout_enq(dbc);
	ep_ctx->ep_info		= 0;
	ep_ctx->ep_info2	= aml_dbc_epctx_info2(BULK_OUT_EP, 1024, max_burst);
	ep_ctx->deq		= cpu_to_le64(deq | dbc->ring_out->cycle_state);

	/* Populate bulk in endpoint context: */
	ep_ctx			= aml_dbc_bulkin_ctx(dbc);
	deq			= aml_dbc_bulkin_enq(dbc);
	ep_ctx->ep_info		= 0;
	ep_ctx->ep_info2	= aml_dbc_epctx_info2(BULK_IN_EP, 1024, max_burst);
...
static void xhci_dbc_giveback(struct aml_dbc_request *req, int status)
	__releases(&dbc->lock)
	__acquires(&dbc->lock)
{
	struct aml_xhci_dbc		*dbc = req->dbc;
	...
	trace_aml_xhci_dbc_giveback_request(req);

	dma_unmap_single(dev,
			 req->dma,
			 req->length,
			 aml_dbc_ep_dma_direction(req));
...
static void xhci_dbc_flush_single_request(struct aml_dbc_request *req)
{
	union aml_xhci_trb	*trb = req->trb;
...
static void xhci_dbc_flush_endpoint_requests(struct aml_dbc_ep *dep)
{
	struct aml_dbc_request	*req, *tmp;

	list_for_each_entry_safe(req, tmp, &dep->list_pending, list_pending)
		xhci_dbc_flush_single_request(req);
}

static void xhci_dbc_flush_requests(struct aml_xhci_dbc *dbc)
{
...
aml_dbc_alloc_request(struct aml_xhci_dbc *dbc, unsigned int direction, gfp_t flags)
{
	...
	trace_aml_xhci_dbc_alloc_request(req);

	return req;
}

void
aml_dbc_free_request(struct aml_dbc_request *req)
{
	trace_aml_xhci_dbc_free_request(req);

	kfree(req);
}

static void
xhci_dbc_queue_trb(struct aml_xhci_ring *ring, u32 field1,
		   u32 field2, u32 field3, u32 field4)
{
	union aml_xhci_trb		*trb, *next;
	...
	trace_aml_xhci_dbc_gadget_ep_queue(ring, &trb->generic);
...
static int xhci_dbc_queue_bulk_tx(struct aml_dbc_ep *dep,
				  struct aml_dbc_request *req)
{
	u64			addr;
	...
	struct aml_xhci_dbc		*dbc = req->dbc;
	struct aml_xhci_ring	*ring = dep->ring;
	u32			length, control, cycle;

	num_trbs = aml_count_trbs(req->dma, req->length);
	WARN_ON(num_trbs != 1);
	...
	req->trb_dma = aml_xhci_trb_virt_to_dma(ring->enq_seg, ring->enqueue);
...
static int
dbc_ep_do_queue(struct aml_dbc_request *req)
{
	int			ret;
	struct aml_xhci_dbc		*dbc = req->dbc;
	struct device		*dev = dbc->dev;
	struct aml_dbc_ep		*dep = &dbc->eps[req->direction];
...
int aml_dbc_ep_queue(struct aml_dbc_request *req)
{
	...
	trace_aml_xhci_dbc_queue_request(req);

	return ret;
}

static inline void xhci_dbc_do_eps_init(struct aml_xhci_dbc *dbc, bool direction)
{
	struct aml_dbc_ep		*dep;
...
static void xhci_dbc_eps_init(struct aml_xhci_dbc *dbc)
{
	xhci_dbc_do_eps_init(dbc, BULK_OUT);
	xhci_dbc_do_eps_init(dbc, BULK_IN);
}

static void xhci_dbc_eps_exit(struct aml_xhci_dbc *dbc)
{
	memset(dbc->eps, 0, sizeof(struct aml_dbc_ep) * ARRAY_SIZE(dbc->eps));
}

static int dbc_erst_alloc(struct device *dev, struct aml_xhci_ring *evt_ring,
		    struct aml_xhci_erst *erst, gfp_t flags)
{
	erst->entries = dma_alloc_coherent(dev, sizeof(struct aml_xhci_erst_entry),
...
static void dbc_erst_free(struct device *dev, struct aml_xhci_erst *erst)
{
	if (erst->entries)
		dma_free_coherent(dev, sizeof(struct aml_xhci_erst_entry),
				  erst->entries, erst->erst_dma_addr);
	erst->entries = NULL;
}

static struct aml_xhci_container_ctx *
...
static struct aml_xhci_ring *
xhci_dbc_ring_alloc(struct device *dev, enum aml_xhci_ring_type type, gfp_t flags)
{
	...
	if (type != TYPE_EVENT) {
		union aml_xhci_trb *trb = &seg->trbs[TRBS_PER_SEGMENT - 1];

		trb->link.segment_ptr = cpu_to_le64(dma);
		trb->link.control = cpu_to_le32(LINK_TOGGLE | TRB_TYPE(TRB_LINK));
	}
	INIT_LIST_HEAD(&ring->td_list);
	aml_xhci_initialize_ring_info(ring, 1);
	return ring;
dma_fail:
	kfree(seg);
seg_fail:
...
static int xhci_dbc_mem_init(struct aml_xhci_dbc *dbc, gfp_t flags)
{
	int			ret;
	dma_addr_t		deq;
	u32			string_length;
	...
	dbc->string_size = sizeof(struct aml_dbc_str_descs);
	...
	deq = aml_xhci_trb_virt_to_dma(dbc->ring_evt->deq_seg,
...
static void xhci_dbc_mem_cleanup(struct aml_xhci_dbc *dbc)
{
	if (!dbc)
		return;
...
static int xhci_do_dbc_start(struct aml_xhci_dbc *dbc)
{
	int			ret;
	u32			ctrl;
	...
	ret = aml_xhci_handshake(&dbc->regs->control,
			     DBC_CTRL_DBC_ENABLE,
			     0, 1000);
	if (ret)
		return ret;
...
static int xhci_do_dbc_stop(struct aml_xhci_dbc *dbc)
{
	if (dbc->state == DS_DISABLED)
		return -1;
...
static int xhci_dbc_start(struct aml_xhci_dbc *dbc)
{
	int			ret;
	unsigned long		flags;
...
static void xhci_dbc_stop(struct aml_xhci_dbc *dbc)
{
	int ret;
	unsigned long		flags;
...
static void
dbc_handle_port_status(struct aml_xhci_dbc *dbc, union aml_xhci_trb *event)
{
	u32			portsc;
...
static void dbc_handle_xfer_event(struct aml_xhci_dbc *dbc, union aml_xhci_trb *event)
{
	struct aml_dbc_ep		*dep;
	struct aml_xhci_ring	*ring;
	int			ep_id;
	...
	struct aml_dbc_request	*req = NULL, *r;
	...
	trace_aml_xhci_dbc_handle_transfer(ring, &req->trb->generic);
...
static void inc_evt_deq(struct aml_xhci_ring *ring)
{
...
static enum evtreturn xhci_dbc_do_handle_events(struct aml_xhci_dbc *dbc)
{
	dma_addr_t		deq;
	struct aml_dbc_ep		*dep;
	union aml_xhci_trb		*evt;
	u32			ctrl, portsc;
	...
	while ((le32_to_cpu(evt->event_cmd.flags) & TRB_CYCLE) ==
		...
		trace_aml_xhci_dbc_handle_event(dbc->ring_evt, &evt->generic);
...
static void xhci_dbc_handle_events(struct work_struct *work)
{
	...
	dbc = container_of(to_delayed_work(work), struct aml_xhci_dbc, event_work);
...
struct aml_xhci_dbc *
aml_xhci_alloc_dbc(struct device *dev, void __iomem *base, const struct aml_dbc_driver *driver)
{
	struct aml_xhci_dbc		*dbc;
	int			ret;
...
/* undo what aml_xhci_alloc_dbc() did */
...
int aml_xhci_create_dbc_dev(struct aml_xhci_hcd *xhci)
{
	...
	ret = aml_xhci_dbc_tty_probe(dev, base + dbc_cap_offs, xhci);
...
void aml_xhci_remove_dbc_dev(struct aml_xhci_hcd *xhci)
{
	...
	aml_xhci_dbc_tty_remove(xhci->dbc);
...
{
	struct aml_xhci_dbc		*dbc = xhci->dbc;
