...
static struct aml_xhci_regset *xhci_debugfs_alloc_regset(struct aml_xhci_hcd *xhci)
{
	struct aml_xhci_regset	*regset;
...
static void xhci_debugfs_free_regset(struct aml_xhci_regset *regset)
{
	if (!regset)
		return;
...
__printf(6, 7)
static void xhci_debugfs_regset(struct aml_xhci_hcd *xhci, u32 base,
				const struct debugfs_reg32 *regs,
				size_t nregs, struct dentry *parent,
				const char *fmt, ...)
{
	struct aml_xhci_regset	*rgs;
	va_list			args;
...
static void xhci_debugfs_extcap_regset(struct aml_xhci_hcd *xhci, int cap_id,
				       const struct debugfs_reg32 *regs,
...
{
	dma_addr_t		dma;
	struct aml_xhci_ring	*ring = *(struct aml_xhci_ring **)s->private;

	dma = aml_xhci_trb_virt_to_dma(ring->enq_seg, ring->enqueue);
	seq_printf(s, "%pad\n", &dma);
...
static int xhci_ring_dequeue_show(struct seq_file *s, void *unused)
{
	...
	dma = aml_xhci_trb_virt_to_dma(ring->deq_seg, ring->dequeue);
	seq_printf(s, "%pad\n", &dma);
...
static struct aml_xhci_file_map ring_files[] = {
...
{
	int			i;
	struct aml_xhci_file_map	*f_map;
...
static int xhci_slot_context_show(struct seq_file *s, void *unused)
{
	...
	struct aml_xhci_slot_priv	*priv = s->private;
	struct aml_xhci_virt_device	*dev = priv->dev;
	char			str[XHCI_MSG_MAX];

	xhci = hcd_to_xhci(bus_to_hcd(dev->udev->bus));
	slot_ctx = aml_xhci_get_slot_ctx(xhci, dev->out_ctx);
	seq_printf(s, "%pad: %s\n", &dev->out_ctx->dma,
...
static int xhci_endpoint_context_show(struct seq_file *s, void *unused)
{
	...
	for (ep_index = 0; ep_index < 31; ep_index++) {
		ep_ctx = aml_xhci_get_ep_ctx(xhci, dev->out_ctx, ep_index);
...
static struct aml_xhci_file_map context_files[] = {
...
{
	struct aml_xhci_port	*port = s->private;
	u32			portsc;
	char			str[XHCI_MSG_MAX];
			       ...
			       size_t count, loff_t *ppos)
{
	...
	struct aml_xhci_port	*port = s->private;
	struct aml_xhci_hcd		*xhci = hcd_to_xhci(port->rhub->hcd);
	char                    buf[32];
	u32			portsc;
	unsigned long		flags;
...
static void xhci_debugfs_create_files(struct aml_xhci_hcd *xhci,
				      struct aml_xhci_file_map *files,
...
static struct dentry *xhci_debugfs_create_ring_dir(struct aml_xhci_hcd *xhci,
...
static void xhci_debugfs_create_context_files(struct aml_xhci_hcd *xhci,
					      struct dentry *parent,
					      int slot_id)
{
...
void aml_xhci_debugfs_create_endpoint(struct aml_xhci_hcd *xhci,
				  struct aml_xhci_virt_device *dev,
				  int ep_index)
{
	struct aml_xhci_ep_priv	*epriv;
	struct aml_xhci_slot_priv	*spriv = dev->debugfs_private;
...
void aml_xhci_debugfs_remove_endpoint(struct aml_xhci_hcd *xhci,
				  struct aml_xhci_virt_device *dev,
				  int ep_index)
{
	struct aml_xhci_ep_priv	*epriv;
...
{
	struct aml_xhci_ep_priv	*epriv = s->private;
...
{
	struct aml_xhci_ep_priv	*epriv = s->private;
	struct aml_xhci_stream_ctx	*stream_ctx;
	dma_addr_t		dma;
	int			id;
...
void aml_xhci_debugfs_create_stream_files(struct aml_xhci_hcd *xhci,
				      struct aml_xhci_virt_device *dev,
				      int ep_index)
{
	struct aml_xhci_slot_priv	*spriv = dev->debugfs_private;
...
void aml_xhci_debugfs_create_slot(struct aml_xhci_hcd *xhci, int slot_id)
{
	struct aml_xhci_slot_priv	*priv;
	struct aml_xhci_virt_device	*dev = xhci->devs[slot_id];
...
void aml_xhci_debugfs_remove_slot(struct aml_xhci_hcd *xhci, int slot_id)
{
	int			i;
	struct aml_xhci_slot_priv	*priv;
	struct aml_xhci_virt_device	*dev = xhci->devs[slot_id];
...
static void xhci_debugfs_create_ports(struct aml_xhci_hcd *xhci,
				      struct dentry *parent)
{
...
void aml_xhci_debugfs_init(struct aml_xhci_hcd *xhci)
{
...
void aml_xhci_debugfs_exit(struct aml_xhci_hcd *xhci)
{
	struct aml_xhci_regset	*rgs, *tmp;
...
void __init aml_xhci_debugfs_create_root(void)
{
	xhci_debugfs_root = debugfs_create_dir("xhci-aml", usb_debug_root);
}

void __exit aml_xhci_debugfs_remove_root(void)
{
