...
EXPORT_SYMBOL_GPL(aml_sb800_prefetch);
...
EXPORT_SYMBOL_GPL(aml_usb_hcd_amd_remote_wakeup_quirk);
...
EXPORT_SYMBOL_GPL(aml_usb_amd_hang_symptom_quirk);
...
EXPORT_SYMBOL_GPL(aml_usb_amd_prefetch_quirk);
...
EXPORT_SYMBOL_GPL(aml_usb_amd_quirk_pll_check);
...
EXPORT_SYMBOL_GPL(aml_usb_amd_quirk_pll_disable);
...
EXPORT_SYMBOL_GPL(aml_usb_asmedia_modifyflowcontrol);

void aml_usb_amd_quirk_pll_enable(void)
{
	usb_amd_quirk_pll(0);
}
EXPORT_SYMBOL_GPL(aml_usb_amd_quirk_pll_enable);
...
EXPORT_SYMBOL_GPL(aml_usb_amd_dev_put);
...
EXPORT_SYMBOL_GPL(aml_usb_amd_pt_check_port);
...
EXPORT_SYMBOL_GPL(aml_uhci_reset_hc);
...
reset_needed:
	...
	aml_uhci_reset_hc(pdev, base);
	return 1;
}
EXPORT_SYMBOL_GPL(aml_uhci_check_and_reset_hc);
...
static void quirk_usb_handoff_uhci(struct pci_dev *pdev)
{
	...
	if (base)
		aml_uhci_check_and_reset_hc(pdev, base);
}
...
void aml_usb_enable_intel_xhci_ports(struct pci_dev *xhci_pdev)
{
	...
	if (!IS_ENABLED(CONFIG_USB_XHCI_HCD)) {
		...
		aml_usb_disable_xhci_ports(xhci_pdev);
		return;
	}
...
EXPORT_SYMBOL_GPL(aml_usb_enable_intel_xhci_ports);
...
EXPORT_SYMBOL_GPL(aml_usb_disable_xhci_ports);
...
hc_init:
	if (pdev->vendor == PCI_VENDOR_ID_INTEL)
		aml_usb_enable_intel_xhci_ports(pdev);
