...
static int xhci_create_intel_xhci_sw_pdev(struct aml_xhci_hcd *xhci, u32 cap_offset)
{
	...
	if (!pdev) {
		aml_xhci_err(xhci, "couldn't allocate %s platform device\n",
			 USB_SW_DRV_NAME);
		return -ENOMEM;
	}
...
EXPORT_SYMBOL_GPL(aml_xhci_ext_cap_init);
