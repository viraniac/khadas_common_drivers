 ...
 * hdmirx_open_port
 ***********************/
static void hdmirx_open_main_port(u8 port)
{
	u32 fsmst = sm_pause;
...
void hdmirx_open_port(u8 main_port, u8 sub_port)
{
	...
	if (rx_is_pip_on() && (sub_port == 0x1 || sub_port == 0x0))
...
{
	if (rx[port].tx_type == DEV_UNKNOWN)
		return false;
	else if (rx[port].tx_type == SPD_GET_EARLIER)
