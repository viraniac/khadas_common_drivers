...
static int s1a_dmc_mon_irq(struct dmc_monitor *mon, void *data, char clear)
{
	unsigned long irqreg;
	struct dmc_mon_comm *mon_comm = (struct dmc_mon_comm *)data;
...
static void s1a_dmc_vio_to_port(unsigned long status, int *port,
...
static int s1a_reg_analysis(char *input, char *output)
{
	...
	if (status & 0x1) { /* read */
		...
		subport = vio_reg3 & 0x7;
		if (subport == 0x2)
			subport = (vio_reg3 >> 7) & 0x3;
	...
	if (status & 0x2) { /* write */
		...
		subport = vio_reg1 & 0x7;
		if (subport == 0x2)
			subport = (vio_reg1 >> 7) & 0x3;
...
static int s1a_dmc_reg_control(char *input, char control, char *output)
{
	int count = 0, i;
	unsigned long val;
	...
	case 'a':	/* analysis sec vio reg */
		count = s1a_reg_analysis(input, output);
		break;
