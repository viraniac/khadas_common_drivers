...
static const struct cpu_dyn_table t5w_cpu_dyn_table[] = {
...
static struct t5w_sys_pll_nb_data t5w_sys_pll_nb_data = {
	...
	.cpu_dyn_clk = &t5w_cpu_dyn_clk.hw,
...
static struct clk_hw_onecell_data t5w_hw_onecell_data = {
	.hws = {
		...
		[CLKID_CPU_CLK_DYN]		= &t5w_cpu_dyn_clk.hw,
		[CLKID_CPU_CLK]			= &t5w_cpu_clk.hw,
		[CLKID_DSU_CLK_DYN]		= &t5w_dsu_dyn_clk.hw,
...
static int __ref meson_t5w_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < t5w_hw_onecell_data.num; i++) {
		...
		ret = devm_clk_hw_register_clkdev(dev, t5w_hw_onecell_data.hws[i],
					NULL, clk_hw_get_name(t5w_hw_onecell_data.hws[i]));
		if (ret < 0) {
